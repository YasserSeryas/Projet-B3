...
export async function validateEmail(ctx) {
    try {
        const tokenValidationSchema = Joi.object({
            token: Joi.string().required(),
        });
        const params = ctx.request.body;
        const { error, value } = tokenValidationSchema.validate(params);
        const user = await UserModel.findOne(value);
        if (!user) throw new Error("Invalid token");
        user.settings.is_email_validated = true;
        user.settings.validation_email_token = null;
        await user.save();
        ctx.ok({ message: "Email validated" });
    } catch (e) {
        ctx.badRequest({ message: e.message });
    }
}