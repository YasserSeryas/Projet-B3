...
const Home = () => {
  ...
  return (
    <>
      ...
      <div className="card" onClick={() => navigate("/menCategory/")}>
      <h1 className='text-1xl font-bold'>Man</h1>
        </div>
        <div className="card" onClick={() => navigate("/womenCategory/")}>
      <h1 className='text-1xl font-bold'>Woman</h1>
        </div>
      ...
      <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        ...
        <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8" >
          {produits.map((produit,index) => (
            <div  key={index} className="card" onClick={() => navigate("/product/" + produit._id)}>
